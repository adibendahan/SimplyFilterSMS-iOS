...
- **FilterImportExportManager** — Merge-only filter import/export (`.sfsfilters`). Holds one in-flight import (`pendingPreview`); Home presents `.filterImport`. Writes/deletes export files in the temp directory.
...
- **Screens/** — SwiftUI views: `AppHomeView`, `FilterListView`, `AddFilterView`, `TestFiltersView`, `LanguageListView`, `AboutView`, `HelpView`, `ChooseAccentColorView`, `ReportMessageView`, `EnableExtensionVideoView`, `WhatsNewView`, `TipJarView`, `FilterImportPreviewView`.
