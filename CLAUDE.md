...
- **SchedulingManager** — Background `BGProcessingTask` for AI list refresh, monthly inactivity reminder, and inactivity notification permission after the Home alert. Uses `UserNotificationCenterService` as a dumb UN pipe.
...
- **FlowManager** — Launch-order queue (not a navigator). Occupancy: `next()` sets `activeScreen`; further `next()` is nil until `complete`. Order: first run → launch (file/deep link) → automatic What's New → inactivity notification → user `request`.
