...
- **New files**: `TipJarView.swift` (view + viewmodel + TipCard + TipCardButtonStyle), `ConfettiView.swift` (`UIViewRepresentable` wrapper), `TipJar.storekit` config file
- **Modified files**: `Screen.swift` (new case), `AppHomeView.swift` (menu entry point), `AboutView.swift` (link entry point), `Constsants.swift` (`TipTier` enum with `tierDescription`, `iconColor`, `fallbackPrice`), `NotificationView.swift` (`tipSuccessful` case, `onHide` callback), `Localizable.strings` (new keys in en + he)
