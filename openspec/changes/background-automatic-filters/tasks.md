...
- [x] 1.3 Add explainer ask-count / last-declined-session / was-granted defaults (and max/gap constants); remove one-shot `didShow…` bool
...
- [x] 2.6 Home calls `evaluateNotificationPermissionExplainer`; on `.show` enable FlowManager + present (Stop Asking on final ask); grant/revoke handled in `SchedulingManager`
...
- [x] 5.1 Tests: reminder schedule/cancel; explainer evaluate cadence (gap, final ask, max, grant, revoke); Home wires evaluate/dismiss/continue
